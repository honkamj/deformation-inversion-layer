...
def _move_channels_last(tensor: Tensor, num_channel_dims: int = 1) -> Tensor:
    ...
    return tensor.permute(
        [0]
        + list(range(num_channel_dims + 1, tensor.ndim))
        + list(range(1, num_channel_dims + 1))
    )


@script
def _index_by_channel_dims(
    n_total_dims: int, channel_dim_index: int, n_channel_dims: int
) -> int:
...
def _convert_voxel_to_normalized_coordinates(
    ...
    channel_dim = _index_by_channel_dims(
        coordinates.ndim, channel_dim_index=0, n_channel_dims=1
    )
    ...
    inferred_volume_shape = (
        coordinates.shape[-n_dims:] if volume_shape is None else volume_shape
    )
    ...
    coordinate_grid_start = tensor(
        -1.0, dtype=coordinates.dtype, device=coordinates.device
    ).view(add_spatial_dims_view)
    coordinate_grid_end = tensor(
        1.0, dtype=coordinates.dtype, device=coordinates.device
    ).view(add_spatial_dims_view)
    output = (
        coordinates
...
def interpolate(
    ...
    normalized_grid = _convert_voxel_to_normalized_coordinates(
    ...
    permuted_volume, permuted_grid = _broadcast_batch_size(
        permuted_volume, permuted_grid
    )
    return grid_sample(

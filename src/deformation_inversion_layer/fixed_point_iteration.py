...
class MaxElementWiseAbsStopCriterion(BaseStopCriterion):
    """Criterion which stops when no element-wise difference is larger than a threshold
    
    Args:
        min_iterations: Minimum number of iterations to use
        max_iterations: Maximum number of iterations to use
        threshold: Error threshold for stopping the iteration
    """

    def __init__(
        self, min_iterations: int = 2, max_iterations: int = 50, threshold: float = 1e-2
    ) -> None:
...
class RelativeL2ErrorStopCriterion(BaseStopCriterion):
    """Criterion which stops when relative L^2 error is below a threshold

...
class InterpolationTests(TestCase):
    ...
    POINTS = (tensor([0.5, 1.5, 2.0]), tensor([0.0, 0.0, 0.0]), tensor([1.0, 2.0, 3.0]))
    VALUES = (tensor((7 + 11 + 19 + 23) / 4), tensor(1.0), tensor(24.0))
    ...
    def test_consistency_with_grid_generation(self) -> None:
        ...
        assert_close(interpolate(voxel_grid, grid=voxel_grid), voxel_grid)
    ...
    def test_shape_consistency_for_interpolaton(self) -> None:
        ...
        for grid_shape, volume_shape, target_shape in zip(
            self.GRID_SHAPES, self.VOLUME_SHAPES, self.TARGET_SHAPES
        ):
            grid = rand(*grid_shape) * 30
            volume = rand(*volume_shape)
            interpolated = interpolate(volume, grid=grid)
            assert_close(interpolated.shape, target_shape)
            ...
            for grid, target in zip(self.POINTS, self.VALUES):
                ...
                for (
                    ...
                    if target.ndim > 1:
                        ...
                        target = target[None]
                    assert_close(interpolate(batched_volume, grid), target)

...
class BroadcastShapeTestingUtil:
    ...
    def _broadcast_to_by_leading_dims(
        ...
        dims_to_add = cls._num_spatial_dimensions(
            len(shape), num_leading_dims
        ) - cls._num_spatial_dimensions(tensor.ndim, num_leading_dims)
        new_shape = (
            tensor.shape[: anchor_dim + 1]
            + (1,) * dims_to_add
            + tensor.shape[anchor_dim + 1 :]
        )
        if new_shape:
    ...
    def expand_tensor_shapes_for_testing(
        ...
        shape_iterator = chain(
            product(cls.BATCH_SHAPES, cls.SPATIAL_SHAPES), [(tuple(), tuple())]
        )
